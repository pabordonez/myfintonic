...
CREATE TABLE IF NOT EXISTS `FinancialEntity` (
    ...
    PRIMARY KEY (`id`),
    UNIQUE INDEX `idx_financial_entity_name`(`name`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Tabla de Relación Cliente-Entidad Financiera
CREATE TABLE IF NOT EXISTS `ClientFinancialEntity` (
    `id` VARCHAR(191) NOT NULL,
    ...
    PRIMARY KEY (`id`),
    UNIQUE INDEX `idx_cfe_client_entity`(`clientId`, `financialEntityId`),
    CONSTRAINT `fk_cfe_client` FOREIGN KEY (`clientId`) REFERENCES `Client`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk_cfe_entity` FOREIGN KEY (`financialEntityId`) REFERENCES `FinancialEntity`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Tabla de Histórico de Valor de Entidades del Cliente
CREATE TABLE IF NOT EXISTS `ClientFinancialEntityValueHistory` (
    ...
    PRIMARY KEY (`id`),
    INDEX `idx_cfe_history_entity`(`clientFinancialEntityId`),
    CONSTRAINT `fk_cfe_history_entity` FOREIGN KEY (`clientFinancialEntityId`) REFERENCES `ClientFinancialEntity`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
