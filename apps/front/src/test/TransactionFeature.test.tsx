...
describe('Transaction Feature', () => {
  ...
  describe('ProductTransactionButton', () => {
    it('debe mostrar el botón para CURRENT_ACCOUNT', () => {
      renderWithRouter(<ProductTransactionButton productType={ProductType.CURRENT_ACCOUNT} productId="123" />);
      expect(screen.getByText('Ver Transacciones')).toBeInTheDocument();
    });

    it('debe mostrar el botón para SAVINGS_ACCOUNT', () => {
      renderWithRouter(<ProductTransactionButton productType={ProductType.SAVINGS_ACCOUNT} productId="123" />);
      expect(screen.getByText('Ver Transacciones')).toBeInTheDocument();
    });

    it('NO debe mostrar el botón para INVESTMENT_FUND', () => {
      renderWithRouter(<ProductTransactionButton productType={ProductType.INVESTMENT_FUND} productId="123" />);
  ...
  describe('TransactionFormPage Submission', () => {
    it('debe llamar a la API con el payload correcto al guardar', async () => {
      ...
      fireEvent.change(screen.getByLabelText(/fecha/i), { target: { value: dateValue } });
      fireEvent.change(screen.getByLabelText(/descripción/i), { target: { value: 'Salary' } });
      fireEvent.change(screen.getByLabelText(/monto/i), { target: { value: '1500.50' } });
      ...
      await waitFor(() => {
        expect(axios.post).toHaveBeenCalledWith(expect.stringContaining('/products/123/transactions'), {
          description: 'Salary',
          amount: 1500.50,
  ...
  describe('TransactionListPage', () => {
    ...
    it('renderiza la lista de transacciones correctamente', async () => {
      ...
      const mockProduct = {
        ...
        transactions: [
          { date: '2023-10-01T10:00:00Z', description: 'Ingreso Nómina', amount: 2500.00 },
          { date: '2023-10-02T12:00:00Z', description: 'Pago Alquiler', amount: -800.00 }
        ]
      };
      ...
      await waitFor(() => expect(screen.queryByText('Cargando transacciones...')).not.toBeInTheDocument());
    });
    ...
    it('muestra estado vacío y botón de crear primera transacción', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('No hay transacciones registradas para este producto.')).toBeInTheDocument();
        expect(screen.getByText('Crear Primera Transacción')).toBeInTheDocument();
      });
    });
    ...
    it('navega a nueva transacción desde el listado', async () => {
      ...
      expect(mockNavigate).toHaveBeenCalledWith('/products/123/transactions/new');
    });
