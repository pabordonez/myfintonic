...
import {
  ValueHistoryList,
  ValueHistory,
} from '../features/financial-entities/components/ValueHistoryList'
...
describe('ValueHistoryList', () => {
  ...
  it('calculates percentage correctly and applies styles', () => {
    ...
    const positiveElement = screen.getByText((content, element) => {
      return (
        (element?.textContent?.includes('10.00%') &&
          element?.className.includes('text-green-800')) ??
        false
      )
    })
    ...
    const negativeElement = screen.getByText((content, element) => {
      return (
        (element?.textContent?.includes('10.00%') &&
          element?.className.includes('text-red-800')) ??
        false
      )
    })
  ...
  it('calculates and displays total profitability based on initialBalance', () => {
    ...
    render(
      <ValueHistoryList history={mockHistory} initialBalance={initialBalance} />
    )

    // Check for Total badge
    const totalBadge = screen.getByText((content, element) => {
      return (
        (element?.textContent?.includes('Total:10.00%') &&
          element?.className.includes('text-green-800')) ??
        false
      )
    })
