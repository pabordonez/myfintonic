...
describe('AuthUseCases', () => {
  ...
  describe('login', () => {
    ...
    it('should throw if password invalid', async () => {
      vi.mocked(mockClientRepository.findByEmail).mockResolvedValue({ password: 'hash' } as any)
      vi.mocked(bcrypt.compare).mockResolvedValue(false as any)
      await expect(useCases.login({ email: 'test@test.com', password: '123' })).rejects.toThrow('Invalid credentials')
    })

    it('should return token if valid', async () => {
      vi.mocked(mockClientRepository.findByEmail).mockResolvedValue({ id: '1', email: 'test@test.com', password: 'hash', role: 'USER' } as any)
